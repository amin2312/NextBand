...
[Bilibiliに投稿したデモ動画](https://www.bilibili.com/video/BV19qgP6fE4u/)
...
1. 健康・高齢者ケア
   - 異常な心拍数や血中酸素濃度を検出した際に、自動的に家族へ通知します。
   - 事前に定義した健康状態の条件を満たした場合、指定された緊急連絡先へ自動的に連絡します。
2. スポーツ・フィットネス
   - 「心拍数が 160 BPM を超えたら、ワークアウトの強度を下げるようリマインドして。」
   - 心拍数、活動レベル、運動状態に基づいてトレーニングプランを動的に調整します。
3. 睡眠・日常の健康管理
   - 「数晩続けて睡眠中の心拍数が異常に高い場合は、健康診断の予約をリマインドして。」
   - 心拍数や血中酸素濃度などの生体データを分析し、ユーザーの健康状態へのより深い洞察を提供します。
4. 自立生活・個人の安全
   - 「長時間活動がなく、心拍数が異常になった場合は、家族に連絡して。」
   - 転倒検知や GPS などの機能と連携し、安全アラートと緊急支援を自動的に発動します。
5. ストレス・メンタルウェルビーイング
   - 「心拍数が高い状態が続いたら、休憩を取るようリマインドして。」
   - 健康指標の変化を組み合わせて、潜在的なストレスや疲労を特定します。
6. パーソナル AI アシスタント
   - 健康モニタリングを超えて、ウェアラブルを AI エージェントの入口にします。
   - 将来的にはスマートホーム機器や企業システムなどのサービスと接続し、真の「感知 → 推論 → 行動」体験を実現します。
